...
namespace Microsoft.Research.DynamicDataDisplay.Charts.Maps
{
	...
	public class MapMagnifyingGlass : ContentGraph
	{
		...
		protected override void OnPlotterAttached()
		{
			...
			int i = 1;
			while (i < littlePlotter.Children.Count)
			{
				littlePlotter.Children[i].Remove();
			}
		...
		{
			get { return map.NetworkTileServer; }
			set { map.NetworkTileServer = value; }
		}
		...
		{
			Point pos = e.MouseDevice.GetPosition(Plotter2D.Viewport);
		...
		private void UpdatePosition()
		{
			if (Plotter2D != null)
			{
				UpdatePosition(Mouse.GetPosition(Plotter2D.Viewport));
			}
		}
		...
		private void UpdateLittleVisible()
		{
			if (Plotter2D != null)
			{
				UpdateLittleVisible(Mouse.GetPosition(Plotter2D.Viewport));
			}
		}
